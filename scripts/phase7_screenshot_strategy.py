...
def lower_atm(spot, interval): return float(np.floor(spot / interval) * interval)
def upper_atm(spot, interval): return float(np.ceil(spot / interval) * interval)
...
def targets(spot, ticker, start, end):
    ...
    for scheduled in pd.date_range(start, end, freq="D"):
        ...
        rows.append({"ticker":ticker,"scheduled_entry_date":scheduled,"entry_date":entry,
                     ...
                     "spot_timestamp":r.timestamp,"pe_atm":lower_atm(spot_px,rule["interval"]),
                     "ce_atm":upper_atm(spot_px,rule["interval"])})
...
def extract(con, cyc, ticker, leg4_k):
    ...
    for _,c in cyc.iterrows():
        ...
        specs=[
            ("L1","BUY","PE",monthly,float(c.pe_atm)),
            ("L2","SELL","PE",weekly,float(c.ce_atm + 3*rule["interval"])),
            ("L3","BUY","CE",weekly,float(c.ce_atm)),
            ("L4","SELL","CE",monthly,float(c.ce_atm - leg4_k*rule["interval"])),
        ]
        for leg,side,opt,exp,strike in specs:
            wanted.append({"ticker":ticker,"leg":leg,"side":side,"option_type":opt,
                           ...
                           "spot":float(c.spot),"pe_atm":float(c.pe_atm),"ce_atm":float(c.ce_atm),
                           "leg4_itm_strikes":leg4_k,"spot_timestamp":c.spot_timestamp})
...
def demo():
    spot=25049.55; interval=50
    pe=lower_atm(spot,interval); ce=upper_atm(spot,interval)
    return {"spot":spot,"monthly_pe_atm":pe,"weekly_itm_pe":ce+3*interval,
            "weekly_atm_ce":ce,"monthly_ce_1_strike_itm":ce-interval,
            "monthly_ce_2_strikes_itm":ce-2*interval}
...
def main():
    ...
    manifest={"start":a.start,"end":a.end,"primary_variant":"leg4=1","files":{}}
